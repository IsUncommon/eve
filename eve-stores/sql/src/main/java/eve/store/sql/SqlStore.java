...
import is.uncommon.eve.Store;
import is.uncommon.eve.Types;
import is.uncommon.eve.Value;
...
public class SqlStore extends Store {
    ...
    public void set(String key, Value value) {
        ValueQuery query = ValueQuery.queryBuilder()
                .key(key);
        ...
        if (cursor != null && cursor.getCount() > 0) {
            db.update(ValueQuery.TABLE, contentValues, SQLiteDatabase.CONFLICT_REPLACE, ValueQuery.WHERE_KEY, args);
        } else {
    ...
    public <T> T get(String key) {
        ...
        if (cursor != null && cursor.getCount() > 0) {
            cursor.moveToFirst();
            boolean isPrimitive = cursor.getInt(cursor.getColumnIndexOrThrow(ValueQuery.IS_PRIMITIVE)) == 1;
